...
String from = "C:\\gameImage\\";

Missile_img = new ImageIcon(from+"Missile.png").getImage();
Enemy_img = new ImageIcon(from+"enemy.png").getImage();
//이미지 만드는 방식을 ImageIcon으로 변경.

Player_img = new Image[5];
for(int i = 0 ; i < Player_img.length ; ++i){
Player_img[i] = 
new ImageIcon(from+"f15k_" + i + ".png").getImage();
}
//플레이어 애니메이션 표현을 위해 파일이름을 
//넘버마다 나눠 배열로 담는다.

BackGround_img = new ImageIcon(from+"background.png").getImage();
//전체 배경화면 이미지를 받습니다.

Cloud_img = new Image[3];
for(int i = 0 ; i <Cloud_img.length ; ++i){
Cloud_img[i] = 
new ImageIcon(from+"cloud_" + i + ".png").getImage();
}
...
Explo_img[i] = 
new ImageIcon(from+"explo_" + i + ".png").getImage();
}
//폭발 애니메이션 표현을 위해 
...
if ( cnt % 200 == 0 ){ 
en = new Enemy(f_width + 80, 100, enemy_speed);
Enemy_List.add(en); 
en = new Enemy(f_width + 60, 200, enemy_speed);
Enemy_List.add(en);
en = new Enemy(f_width + 20, 300, enemy_speed);
Enemy_List.add(en);
en = new Enemy(f_width + 30, 400, enemy_speed);
Enemy_List.add(en);
en = new Enemy(f_width + 100, 500, enemy_speed);
Enemy_List.add(en);
en = new Enemy(f_width+ 70, 150, enemy_speed);
