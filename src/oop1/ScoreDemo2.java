	...
	public static void main(String[] args) {
		...
		while(true) {
			...
			try {
				...
				System.out.println("잘못된 입력입니다. 정확한 번호를 입력해주세요");
			...
			//삭제	
				...
				if(foundScore != null) {
					System.out.println("해당 학생의 데이터를 삭제하시겠습니까? (예,아니오) : ");
					String confirm = sc.next();
					if(confirm.equals("예")) {
						
					for(int i = 0; i < scores.length; i++) {
						if(scores[i] == foundScore) {
					...
					savePosition--;
						...
						System.out.println("정확하지 않은 명령어입니다. 다시 입력해주세요");
					}
				}else {
					System.out.println("["+searchName+"] 과 일치하는 학생정보를 찾을 수 없습니다.");
				}
